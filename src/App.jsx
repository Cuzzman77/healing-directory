...
const QuickFilters = ({ onFilter, activeFilter, showSaved = true }) => {
    const filters = [ { name: "Brain Health", icon: Brain }, { name: "Immunity", icon: Shield }, { name: "Detox", icon: Sparkles }, { name: "Energy", icon: Zap }, { name: "Gut Health", icon: Activity } ];
    return (
        <div className="flex overflow-x-auto space-x-3 py-2 px-1 scrollbar-hide mb-4 justify-start md:justify-center">
            {filters.map((f) => (<button key={f.name} onClick={() => onFilter(f.name)} className={`flex items-center px-4 py-2 rounded-full shadow-sm text-sm font-semibold border whitespace-nowrap transition-all flex-shrink-0 ${activeFilter === f.name ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-emerald-700 border-emerald-100 hover:bg-emerald-50 hover:border-emerald-200'}`}><f.icon className={`w-4 h-4 mr-2 ${activeFilter === f.name ? 'text-white' : 'text-emerald-500'}`} />{f.name}</button>))}
            {showSaved && (<button onClick={() => onFilter('favorites')} className={`flex items-center px-4 py-2 rounded-full shadow-sm text-sm font-semibold border whitespace-nowrap transition-all flex-shrink-0 ${activeFilter === 'favorites' ? 'bg-red-50 text-red-600 border-red-200' : 'bg-white text-gray-600 border-gray-200 hover:bg-red-50 hover:text-red-500'}`}><Heart className={`w-4 h-4 mr-2 ${activeFilter === 'favorites' ? 'fill-current' : ''}`} />My Saved</button>)}
        </div>
    );
};
    ...
    return (
        <div className="w-full mb-2"> 
            <div className={`border rounded-xl shadow-sm bg-white overflow-hidden`}>
                ...
                {isOpen && (<div className="p-4 bg-gray-50 animate-in slide-in-from-top-2 border-t border-gray-100">
                    {/* TOGGLE BAR MOVED INSIDE */}
                    <div className="flex border border-gray-200 rounded-lg overflow-hidden mb-4">
                        <button onClick={() => { onBrowseModeChange('title'); onSelect(null); }} className={`flex-1 py-2 text-xs font-bold flex items-center justify-center transition-colors ${browseMode === 'title' ? 'bg-emerald-200 text-emerald-800' : 'bg-white text-gray-700 hover:bg-gray-50'}`}><Pill className={`w-3.5 h-3.5 mr-1.5 ${browseMode === 'title' ? 'text-emerald-600' : 'text-gray-400'}`} />By Protocol</button>
                        <div className="w-px bg-gray-200"></div>
                        <button onClick={() => { onBrowseModeChange('ailment'); onSelect(null); }} className={`flex-1 py-2 text-xs font-bold flex items-center justify-center transition-colors ${browseMode === 'ailment' ? 'bg-emerald-200 text-emerald-800' : 'bg-white text-gray-700 hover:bg-gray-50'}`}><LayoutList className={`w-3.5 h-3.5 mr-1.5 ${browseMode === 'ailment' ? 'text-emerald-600' : 'text-gray-400'}`} />By Ailment</button>
                    </div>
                    <div className="flex flex-wrap gap-2 justify-center"><button onClick={() => { onSelect(null); setIsOpen(false); }} className={`px-4 py-2 text-xs font-bold rounded-lg transition-all duration-200 border ${!selected ? 'bg-emerald-600 text-white border-emerald-600 shadow-md transform scale-105' : 'bg-white text-gray-500 border-gray-200 hover:bg-emerald-50 hover:border-emerald-200'}`}>ALL</button>{alphabet.map(char => (<button key={char} onClick={() => { onSelect(char); setIsOpen(false); }} className={`w-9 h-9 flex items-center justify-center text-xs font-bold rounded-lg transition-all duration-200 border ${selected === char ? 'bg-emerald-600 text-white border-emerald-600 shadow-md transform scale-105' : 'bg-white text-gray-500 border-gray-200 hover:bg-emerald-50 hover:border-emerald-200'}`}>{char}</button>))}</div></div>)}
            </div>
        </div>
    )
};
...
const ProtocolDetailPage = ({ protocol, onBack, onShare, db, userId, isFavorite, onToggleFavorite, scrollToTestimonialsOnMount }) => {
    ...
    useEffect(() => {
        ...
        const incrementView = async () => { try { const ref = doc(db, COLLECTION_NAME, protocol.id); await updateDoc(ref, { "popularity_metrics.site_views": increment(1) }); } catch (err) { console.log("View increment suppressed"); } };
        incrementView();
    }, [protocol?.id]);
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            if (fetchedTestimonials.length > 0) setTestimonials(fetchedTestimonials); else setTestimonials((Array.isArray(protocol.testimonials) ? protocol.testimonials : []));
        });
        return () => unsubscribe();
    }, [protocol?.id, userId]);
    const scrollToTestimonials = () => { const element = document.getElementById('testimonials-section'); if (element) element.scrollIntoView({ behavior: 'smooth' }); };
    ...
    const handleSubmitTestimonial = async () => {
        ...
        try {
            await addDoc(collection(db, `${COLLECTION_NAME}/${protocol.id}/testimonials`), { text: testimonialText, score: testimonialScore, userId, date: new Date().toISOString().split('T')[0], user: `User-${userId.substring(0, 4)}`, ailment: testimonialAilment });
            setSubmissionStatus('success'); setTestimonialText(''); setTimeout(() => setSubmissionStatus(null), 3000);
        } catch (e) { setSubmissionStatus('error'); }
    };
    return (
        <div className="p-4 sm:p-6 bg-white rounded-2xl shadow-2xl min-h-[80vh] animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <section id="vendors-section" className="mt-8 pt-6 border-t border-gray-200"><h3 className="text-2xl font-bold text-gray-800 mb-4">Where to Buy</h3><div className="space-y-3">{protocol.vendors?.map((vendor, index) => (<div key={index} className="flex justify-between p-4 bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all"><div className="flex items-center gap-4"><p className="font-bold text-gray-900">{vendor.name}</p></div><a href={vendor.link || '#'} target="_blank" rel="noopener noreferrer" className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold rounded-lg">Visit Store</a></div>))}</div></section>
            <section id="testimonials-section" className="mt-8 pt-6 border-t border-gray-200"><h3 className="text-2xl font-bold text-gray-800 mb-4">Protocol Testimonials</h3>{/* ... (Testimonial Form Omitted for brevity) ... */}</section>
        </div>
    );
};
...
const App = () => {
    ...
    const filteredProtocols = useMemo(() => {
        ...
        // If browsing by letter, FORCE Alphabetical Sort
        if (selectedLetter) {
            ...
            switch (sortBy) {
                ...
                case 'popular': results.sort((a, b) => ((Number(a.popularity_metrics?.baseline_report_volume) || 0) + (Number(a.popularity_metrics?.site_views) || 0)) - ((Number(b.popularity_metrics?.baseline_report_volume) || 0) + (Number(b.popularity_metrics?.site_views) || 0))).reverse(); break;
                case 'alpha': results.sort(sortAlpha); break;
            }
        }
        return results;
    }, [protocols, searchTerm, selectedLetter, activeFilter, favorites, browseMode, sortBy]);

    const handleSelectProtocol = useCallback((id) => { setSelectedProtocolId(id); safePushState(null, '', `/protocol/${id}`); window.scrollTo(0, 0); }, []);
    const handleFilter = useCallback((tag) => { setActiveFilter(prev => prev === tag ? null : tag); setSearchTerm(''); setHeroSearchTerm(''); setSelectedLetter(null); setIsBrowsing(true); }, []);
    ...
    const triggerSearch = useCallback((e) => {
        ...
        if (isCommit && heroSearchTerm.trim()) {
            ...
            setTimeout(() => {
                setSearchTerm(heroSearchTerm); setIsBrowsing(true); setSelectedLetter(null); setActiveFilter(null);
            }, 50); 
        }
    }, [heroSearchTerm]);
                        ...
                        <div className="flex-1 min-h-[50vh]">
                            {filteredProtocols.length > 0 ? (
                                <div className="grid grid-cols-1 gap-5 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
                                    <div className="text-xs text-gray-500 font-medium flex justify-between items-center px-1"><span>{filteredProtocols.length} results {selectedLetter && ` starting with "${selectedLetter}"`} {activeFilter === 'favorites' && '(Saved Items)'}</span></div>
                                    {filteredProtocols.map((protocol) => (<ProtocolCard key={protocol.id} protocol={protocol} onSelect={handleSelectProtocol} onShare={handleShare} isFavorite={isFavorite(protocol.id)} onToggleFavorite={toggleFavorite} />))}
                                </div>
                            ) : (
                                <div className="text-center p-12 bg-white rounded-2xl shadow-md border border-gray-100 mt-4"><FlaskConical className="w-12 h-12 mx-auto text-gray-300 mb-3" /><h3 className="text-lg font-bold text-gray-800 mb-1">No results found</h3><button onClick={() => { setActiveFilter(null); startBrowsing(); }} className="mt-4 text-emerald-600 font-bold hover:underline">Browse All Protocols</button></div>
                            )}
                        </div>
                    </div>
                ) : (
                    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700">
                        ...
                        <section>
                            <div className="grid md:grid-cols-3 gap-6">
                                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 text-center"><div className="w-12 h-12 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-4"><Shield className="w-6 h-6 text-blue-600" /></div><h3 className="font-bold text-gray-900 mb-2 flex items-center justify-center">Trust Scores</h3><p className="text-sm text-gray-500">We cut through the noise by separating anecdotal success from scientific validation.</p></div>
        ...
        <div className="min-h-screen bg-gray-50 font-sans p-4 sm:p-6 relative">
            <header className="max-w-xl mx-auto pb-4 mb-2 text-center">
                <button onClick={handleGoHome} className="inline-flex items-center space-x-2 mb-1 hover:opacity-80 transition-opacity">
                    <div className="bg-emerald-600 p-1.5 rounded-full shadow-md"><img src="/logo.png" alt="Healing Directory Logo" className="w-8 h-8 object-contain" /></div>
